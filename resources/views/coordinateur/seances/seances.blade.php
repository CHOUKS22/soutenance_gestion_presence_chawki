                    ...
                    <tbody class="divide-y divide-gray-200">
                        @foreach ($seances as $seance)
                            @php
                                ...
                                $hasPresencesOrAbsences = $seance->presences->isNotEmpty() || $seance->absences->isNotEmpty();
                            @endphp
                            <tr class="{{ $isToday ? 'bg-blue-50' : '' }}">
                                <td class="px-6 py-4">
                                    <div>
                                        <span class="font-medium">{{ $dateDebut->format('d/m/Y') }}</span>
                                        <span class="block text-xs text-gray-500">{{ $dateDebut->format('H:i') }} - {{ $dateFin->format('H:i') }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4">{{ $seance->classe->nom ?? '-' }}</td>
                                <td class="px-6 py-4">{{ $seance->matiere->nom ?? '-' }}</td>
                                <td class="px-6 py-4">{{ $seance->professeur->user->prenom ?? '' }} {{ $seance->professeur->user->nom ?? '' }}</td>
                                <td class="px-6 py-4">{{ $seance->typeSeance->nom ?? '-' }}</td>
                                <td class="px-6 py-4">{{ $seance->statutSeance->libelle ?? '-' }}</td>
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end items-center gap-2">
                                        ...
                                        <span class="w-3 h-3 rounded-full {{ $hasPresencesOrAbsences ? 'bg-green-500' : 'bg-gray-400' }}"></span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-right space-x-2">
                                    <a href="{{ route('seances.show', $seance) }}" class="text-blue-600 hover:text-blue-900">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <a href="{{ route('seances.edit', $seance) }}" class="text-yellow-600 hover:text-yellow-900">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <form action="{{ route('seances.destroy', $seance) }}" method="POST" class="inline-block"
