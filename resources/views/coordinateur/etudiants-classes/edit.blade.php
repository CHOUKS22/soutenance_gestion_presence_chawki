        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            <form method="POST" action="{{ route('etudiants-classes.update', $inscription->id) }}">
                ...
                <div class="mt-6 p-4 bg-gray-50 rounded-lg">
                    <p class="text-sm font-medium text-gray-700 mb-2">Informations actuelles:</p>
                    <p class="text-sm text-gray-600">
                        Étudiant: {{ $inscription->etudiant->user->nom }} {{ $inscription->etudiant->user->prenom }}<br>
                        Classe: {{ $inscription->anneeClasse->classe->nom }} -
                        {{ $inscription->anneeClasse->anneeAcademique->libelle }}<br>
