...
@section('content')
    ...
    <div class="bg-white p-6 rounded shadow space-y-6">
        {{-- Formulaire de taux de présence par etudiant par periode --}}
        <form method="GET" action="{{ route('statistiques.taux-presence-etudiant') }}" class="space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <select name="periode" id="periode" onchange="handlePeriodeChange(this.value)"
                        ...
                        <option value="personnalisee" {{ $periode == 'personnalisee' ? 'selected' : '' }}>Période
        ...
        {{-- Affichage des taux de presence --}}
        @if ($etudiantId)
            <div class="mt-6">
                ...
                @if ($taux !== null)
                    <table class="min-w-full bg-white border border-gray-200">
                        ...
                        <tbody>
                            <tr>
                                <td class="px-4 py-2 border">{{ $etudiants->firstWhere('id', $etudiantId)?->user->nom }}
                                    {{ $etudiants->firstWhere('id', $etudiantId)?->user->prenom }}</td>
                                <td class="px-4 py-2 border">{{ $dateDebut->format('d/m/Y') }} au
                                    {{ $dateFin->format('d/m/Y') }}</td>
                                <td class="px-4 py-2 border">{{ $totalSeances }}</td>
                                <td class="px-4 py-2 border">{{ $totalPresences }}</td>
                                <td
                                    class="px-4 py-2 border font-bold {{ $taux < 30 ? 'text-red-600' : 'text-green-600' }}">
                                    {{ $taux }}%</td>
                            </tr>
                        </tbody>
                    </table>
