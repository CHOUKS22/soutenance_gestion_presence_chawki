            ...
            <div class="bg-white p-4 rounded-2xl shadow space-y-4 text-sm text-gray-700">
                ...
                <div class="flex justify-between">
                    ...
                    <span class="text-yellow-700 font-semibold">
                        {{ $noteAssiduite !== null ? $noteAssiduite : '—' }} / 20
                    </span>
                </div>
            </div>
        </div>
                ...
                <div class="flex items-center justify-between mb-4">
                    ...
                    <form method="GET" action="{{ route('etudiant.dashboard') }}" class="flex items-center space-x-2">
                        ...
                        <input type="week" name="semaine" id="semaine"
                            value="{{ request('semaine', now()->format('Y-\WW')) }}"
                            class="border rounded px-2 py-1 text-sm">
                        <button type="submit"
                            class="bg-blue-600 text-white px-3 py-1 rounded text-sm hover:bg-blue-700">Voir</button>
                    </form>
                </div>
                    ...
                    <ul class="list-disc ml-5 text-sm text-green-800 space-y-1">
                        @foreach ($presences as $presence)
                            <li>
                                ...
                                de {{ \Carbon\Carbon::parse($presence->seance->date_debut)->format('H:i') }} à
                                {{ \Carbon\Carbon::parse($presence->seance->date_fin)->format('H:i') }}
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="bg-blue-50 border-l-4 border-blue-400 p-5 rounded-xl shadow">
                    <h3 class="text-lg font-bold text-blue-800 mb-3">
                        <i class="fas fa-file-alt mr-2"></i> Absences justifiées
                    </h3>
                        ...
                        <ul class="list-disc ml-5 text-sm text-blue-800 space-y-1">
                            @foreach ($absencesJustifiees as $absence)
                                <li>
                                    ...
                                    de {{ \Carbon\Carbon::parse($absence->seance->date_debut)->format('H:i') }} à
                                    {{ \Carbon\Carbon::parse($absence->seance->date_fin)->format('H:i') }}
                                </li>
                            @endforeach
                        </ul>
                    @endif
                ...
                <div class="bg-red-50 border-l-4 border-red-400 p-5 rounded-xl shadow">
                    <h3 class="text-lg font-bold text-red-800 mb-3">
                        <i class="fas fa-times-circle mr-2"></i> Absences non justifiées
                    </h3>
                    ...
                    {{-- Pagination des absences (justifiées et non justifiées) --}}
                    <div class="mt-4">
                        {{ $absences->links() }}
                    </div>
                </div>
